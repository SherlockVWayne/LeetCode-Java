...
// Time Complexity : O(N^2) , N = length of transaction array, we loop for each transaction and compare against all for a given user
// Space Complexity: O(N) , N = length of transaction array (we are storing each transaction in the map)